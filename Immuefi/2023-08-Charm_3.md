# Majority of liquidity added to Uniswap v3 Pool can be burnt by large flashloan.

High

## Bug Description
Let's say current `totalSupply` of alpha vault is 2% of `maxTotalSupply`.
...
https://github.com/charmfinance/alpha-vaults-v2-contracts/blob/main/contracts/AlphaProVault.sol#L310-L328
```solidity
    function withdraw(
        uint256 shares,
        uint256 amount0Min,
...
If Uniswap v3 pool is burnt, it will sit idle at vault and not generate any fee for shareholders and protocol until rebalancing happens.
...
## Recommendation
1) `withdraw()` should first return any funds sitting at the vault and waiting for rebalance, and if that's not enough, it should burn Uniswap v3 pool.
...
```solidity
    function withdraw(
            ...
            if (amount0 > token0.balanceOf(address(this)))
                sharesToBurn = amount0 * totalSupply - total0;
